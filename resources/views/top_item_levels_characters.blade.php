@foreach ( $characters as $nr => $character )
    <tr id="character{{$character->id}}" class="charRow">
        ...
        <td class="cellDesktop"> <a target="_blank" href="https://tauriwow.com/armory#character-sheet.xml?r={{ $realms[$character->realm] }}&n={{ $character->name }}"> {{ $character->name }} </a></td>
        <td class="cellMobile"> <a target="_blank" href="https://tauriwow.com/armory#character-sheet.xml?r={{ $realms[$character->realm] }}&n={{ $character->name }}"> {{ strlen($character->name) > 6 ? (mb_substr($character->name,0,6) . "..") : $character->name  }} </a></td>
