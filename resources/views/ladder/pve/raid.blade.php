<ul class="nav nav-tabs" role="tablist">
    @foreach ( $difficulties as $index => $difficulty )
        <li class="home-main-tab {{ $index == 0 ? "active" : "" }}" role="presentation"><a href="#difficulty-{{ $index }}" aria-controls="difficulty-{{ $index}}" role="tab" data-toggle="tab">{{ $difficulty["name"] }}</a></li>
    @endforeach
</ul>
            ...
            <table class="table table-bordered table-classes">
                ...
                @foreach( $encounters[$difficulty["id"]] as $encounter )
                    <tr>
                        ...
                        <td class="cellMobile" style="white-space:nowrap;"><a href="{{ URL::to('/ladder/pve/') ."/" . $encounter["encounter_id"] }}">{{ \TauriBay\Encounter::getNameShort($encounter["encounter_id"]) }}</a></td>
